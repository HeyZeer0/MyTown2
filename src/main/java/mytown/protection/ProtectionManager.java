...
public class ProtectionManager {
    ...
    public static void check(TileEntity te) {
        for (SegmentTileEntity segment : segmentsTile.get(te.getClass())) {
            if (!segment.shouldExist(te)) {
                ...
                /*if(ProtectionHandlers.instance.getOwnerForTileEntity(te).getPlayer() != null) {
                    ChatManager.send(ProtectionHandlers.instance.getOwnerForTileEntity(te).getPlayer()
                            , "mytown.protection.tileremoved",
                            te.blockType.toString(),
                            ("X: " +String.valueOf(te.xCoord) + ", Y: " + String.valueOf(te.yCoord)  + ", Z: " + String.valueOf(te.zCoord)));
                }*/
                te.getWorldObj().setBlock(te.xCoord, te.yCoord, te.zCoord, Blocks.air);
                te.invalidate();
                te.getWorldObj().setBlock(te.xCoord, te.yCoord, te.zCoord, GameData.getBlockRegistry().getObject("ExtraUtilities:chestMini"));
                IInventory bau = (IInventory)te.getWorldObj().getTileEntity(te.xCoord, te.yCoord, te.zCoord);

                bau.setInventorySlotContents(0, itemStack);
    ...
    public static void checkBlockInteraction(Resident res, BlockPos bp, PlayerInteractEvent.Action action, Event ev) {
        ...
        if(block.getUnlocalizedName().contains("universalcoins")) {
            return;
        }

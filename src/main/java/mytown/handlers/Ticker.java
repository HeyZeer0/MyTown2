...
public class Ticker {
    ...
    public void onTickEvent(TickEvent.WorldTickEvent ev) {
        ...
        if(MyTownUniverse.instance != null && MyTownUniverse.instance.residents != null) {
            ...
            if (lastCalendarDay != -1 && Calendar.getInstance().get(Calendar.DAY_OF_YEAR) != lastCalendarDay) {
                for (int i = 0; i < MyTownUniverse.instance.towns.size(); i++) {
                    ...
                    if (!(town instanceof AdminTown)) {
                        ...
                        if (thirtyDaysAgo.toInstant().isAfter(town.residentsMap.getMayor().getLastOnline().toInstant())) {
                            MyTown.instance.LOG.info("Town {} has been deleted because the owner is logged out for 30 days.", town.getName());
    ...
    public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent ev) {
        ...
        if (res != null) {
           ...
           // MyTown.instance.datasource.changeResidentDate(res);
        } else {

...
public class MyTownDatasource extends DatasourceSQL {
    ...
    public boolean changeResidentDate(Resident resident) {
        ...
        try {
            if (getUniverse().residents.contains(resident.getUUID())) { // Update
                PreparedStatement updateStatement = prepare("UPDATE " + prefix + "Residents SET lastOnline=? WHERE uuid=?", true);
                updateStatement.setLong(1, new Date().getTime() / 1000L); // Stupid hack...
                updateStatement.setString(2, resident.getUUID().toString());
                updateStatement.executeUpdate();
            }
        } catch (SQLException e) {

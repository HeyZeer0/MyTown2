...
import myessentials.economy.core.universalcoins.UniversalCoinsEconomy;
...
public class Economy {
    public static final String CURRENCY_VAULT = "$UniversalCoins";
    public static final String CURRENCY_UNIVERSALCOINS = "$Vault";
    ...
    public Economy(String costItemName) {
        ...
        if(costItemName.equals(CURRENCY_UNIVERSALCOINS)) {
        ...
        else if(costItemName.equals(CURRENCY_VAULT)) {
    ...
    public boolean takeMoneyFromPlayer(EntityPlayer player, int amount) {
        ...
        if(costItemName.startsWith(CURRENCY_UNIVERSALCOINS)) {
            IEconManager eco = economyManagerForUUID(player.getUniqueID());
            if(eco == null) {
                return false;
            }
            return eco.removeFromWallet(amount);
        }
    ...
    public void giveMoneyToPlayer(EntityPlayer player, int amount) {
        if (costItemName.equals(CURRENCY_FORGE_ESSENTIALS) || costItemName.equals(CURRENCY_VAULT) || costItemName.startsWith(CURRENCY_CUSTOM) || costItemName.startsWith(CURRENCY_UNIVERSALCOINS)) {
            IEconManager eco = economyManagerForUUID(player.getUniqueID());
            if (eco == null)
                return;
